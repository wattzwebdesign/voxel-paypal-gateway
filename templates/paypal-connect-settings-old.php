	...
	<template v-if="config.marketplace.enabled">
		<div class="ts-form-group">
			...
			<select v-model="config.marketplace.fee_type" class="ts-input">
				...
				<option value="conditional"><?php _e( 'Conditional (advanced)', 'voxel-payment-gateways' ); ?></option>
			</select>
		</div>
		...
		<div class="ts-form-group" v-if="config.marketplace.fee_type === 'conditional'">
			<label><?php _e( 'Conditional fee rules', 'voxel-payment-gateways' ); ?></label>
			<div class="fee-conditions">
				...
				>
					<select v-model="condition.type" class="ts-input">
						<option value="percentage"><?php _e( 'Percentage', 'voxel-payment-gateways' ); ?></option>
						<option value="fixed"><?php _e( 'Fixed', 'voxel-payment-gateways' ); ?></option>
					</select>

					<input
						type="number"
						v-model="condition.value"
						placeholder="<?php esc_attr_e( 'Value', 'voxel-payment-gateways' ); ?>"
						step="0.01"
						min="0"
						class="ts-input"
					/>
					...
					<input
						...
						placeholder="<?php esc_attr_e( 'Min amount', 'voxel-payment-gateways' ); ?>"
						step="0.01"
						min="0"
						class="ts-input"
					/>
					...
					<input
						...
						placeholder="<?php esc_attr_e( 'Max amount', 'voxel-payment-gateways' ); ?>"
						step="0.01"
						min="0"
						class="ts-input"
					/>
					...
					>
						<?php _e( 'Remove', 'voxel-payment-gateways' ); ?>
					</button>
				</div>
				...
				<button
					...
					class="ts-button ts-outline"
				>
					<?php _e( 'Add condition', 'voxel-payment-gateways' ); ?>
				</button>
			</div>
			<p class="description">
				<?php _e( 'Define conditional fees based on order amount ranges.', 'voxel-payment-gateways' ); ?>
			</p>
		</div>
